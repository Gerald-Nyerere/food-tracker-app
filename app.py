...
def view(date):
    ...
    if  request.method == 'POST':
        db.execute('insert into food_table (food_id, log_date_id) values (?,?)', [request.form['food-select'], date_result['id']])
        db.commit()
    ...
    #FROM log_date 
   # JOIN food_table ON food_table.log_date_id = log_date.id 
    #JOIN food ON food.id = food_table.food_id 
   ...
   # log_results = log_cur.fetchall()
    ...
    log_cur = db.execute('select food.name, food.protein, food.carbohydrates, food.fat, food.calories from log_date join food_table on food_table.log_date_id = log_date.id join food on food.id =food_table.food_id where log_date.entry_date = ?', [date])
    log_results= log_cur.fetchall()
...
def food():
    ...
    results = cur.fetchall()
    # {} Proteins: {} carbs: {} Fat: {}</h1>'.format(request.form['food-name'], request.form['protein'], request.form['carbohydrates'], request.form['fat'])
